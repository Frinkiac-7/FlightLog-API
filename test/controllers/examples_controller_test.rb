...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { first: @example.first, last: @example.last, text: @example.text, username: @example.username } }, as: :json
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { first: @example.first, last: @example.last, text: @example.text, username: @example.username } }, as: :json
    assert_response 200
  end
